...
class TextGenerationDollyPipeline(Pipeline):
    ...
    def _sanitize_parameters(
        ...
        **generate_kwargs,
        ...
        for key in custom_default_generate_args.keys():
            if key in generate_kwargs:
                pass 
            else:
                generate_kwargs[key] = custom_default_generate_args[key]

        preprocess_params = {}
        # run on GPU by default
        if device is not None: 
            preprocess_params["device"] = device
        else: 
            preprocess_params["device"] = 0
